...
export function updateSelectElements() {
    ...
    roleCells.forEach(cell => {
        ...
        // Oculta el botón de editar si el rol es "Cobrador"
        if (roleText === 'Administrador' || roleText === 'Cobrador') {
            const editButton = row.querySelector('.edit-user-button');
            if (editButton) {
                editButton.classList.add('d-none'); // Oculta el botón de editar
            }
        }
    });
    ...
    estadoCells.forEach(cell => {
        ...
        // Oculta el select si el rol es "Administrador" o "Cobrador"
        const roleCell = row.querySelector('.role-col');
        const roleText = roleCell.querySelector('span').textContent.trim();
        if (roleText === 'Administrador' || roleText === 'Cobrador') {
            if (selectElement) {
                selectElement.style.display = 'none'; // Oculta el select
            }
        }
    });
...
function applyEstadoStyles(cell, estadoText) {
    ...
    if (estadoText === "Activo") {
        estadoElement.style.color = "var(--clr-secondary)"; // Color verde para Activo
        estadoElement.style.fontWeight = "bold";
    } else if (estadoText === "Sin carro") {
        estadoElement.style.color = "var(--clr-primary)"; // Color azul para Sin carro
        estadoElement.style.fontWeight = "bold";
    } else if (estadoText === "Suspendido") {
        estadoElement.style.color = "orange"; // Color naranja para Suspendido
        estadoElement.style.fontWeight = "bold";
